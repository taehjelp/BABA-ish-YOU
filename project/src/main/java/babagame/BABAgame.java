...
//	set shit bro
	private void resetAllBools() { // set alle BABAtile-bools til false (unntak:text)
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
	...
	private void setAllProps(char noun, char property) {
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				if (noun == Character.toLowerCase(tileType(x,y))) {
					board[y][x].setBools(noun, property); // brukar ikkje getTile(x,y) fordi loopen ikkje går out of bounds
				}
			}
		}
	}
	private void setAllNouns(char startNoun, char endNoun) {
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				if (startNoun == Character.toLowerCase(tileType(x,y))) {
					if (endNoun == 't') { // text be special bruv
						board[y][x].setType(Character.toUpperCase(endNoun), startNoun);
					}
					else {
						board[y][x].setType(Character.toUpperCase(endNoun));
					}
				}
			}
		}
...
//	rule time	
	public void updateRules() {
		resetAllBools(); // clearing the properties:)
		for (int y = 0; y < height; y++) { 
			for (int x = 0; x < width; x++) {
				if (board[y][x].isOperator(tileText(x,y))) { //	dersom (x,y) == is-tekst:
					if (getTile(x-1,y).isNoun(tileText(x-1,y)) && getTile(x+1,y).isProperty(tileText(x+1,y))) {
						setAllProps(tileText(x-1,y),tileText(x+1,y)) ;
					} // ikkje else if, fordi fleire kan vere tilfelle:))
					if (getTile(x,y-1).isNoun(tileText(x,y-1)) && getTile(x,y+1).isProperty(tileText(x,y+1))) {
...
//		}
	...
	private void movePos(int x, int y, char dir) { //pos er posisjonen til greia ein vil flytte (pos(X)+1 vil seie 1 trinn vidare i retning dir) 
		updateRules();
		countYou();
		if (youCount == 0) {
			isLOSE = true;
			...
			else if (!nextTile.isSolid()) { // ok detta kjeme prolly te å føre te utslettinga av ikkje-solid ting!!!!
				nextTile.setType(type);     // og det er eit problem for Seinare Meg;)))
				tile.setType(' '); 
			}
